...
async function fixture(profile = 'conductor') {
  ...
  await writeFile(join(root, 'accessibility-standards.config.json'), JSON.stringify({ package: '@verndale/accessibility-standards@2.0.2', profile, routes: 'accessibility-standards.routes.json', outputRoot: 'generated' }));
...
test('sync, no-op, and check are deterministic', async () => {
  ...
  try {
    ...
    assert.equal(provenance.package, '@verndale/accessibility-standards@2.0.2');
    assert.equal(provenance.profile_version, '2.0.2');
    ...
    assert.equal(provenance.ui_pattern_ids.length, 80);
...
test('config, routes, traversal, collisions, and symlink escapes fail closed', async () => {
  ...
  try {
    await writeFile(
      configPath,
      JSON.stringify({
        package: '@verndale/accessibility-standards@2.0.2',
        profile: 'conductor',
...
test('sync rejects a consumer pin that does not match the installed contract version', async () => {
  ...
  try {
    ...
    await assert.rejects(() => projection({ configPath }), /Exact installed package pin required: @verndale\/accessibility-standards@2\.0\.2/);
  } finally { await rm(root, { recursive: true, force: true }); }
});
