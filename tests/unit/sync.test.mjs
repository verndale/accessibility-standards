...
async function fixture(profile = 'conductor') {
  ...
  await writeFile(join(root, 'accessibility-standards.config.json'), JSON.stringify({ package: '@verndale/accessibility-standards@3.1.0', profile, routes: 'accessibility-standards.routes.json', outputRoot: 'generated' }));
...
test('sync, no-op, and check are deterministic', async () => {
  ...
  try {
    ...
    assert.equal(provenance.package, '@verndale/accessibility-standards@3.1.0');
    assert.equal(provenance.profile_version, '3.1.0');
...
test('sync rejects a consumer pin that does not match the installed contract version', async () => {
  ...
  try {
    ...
    await assert.rejects(() => projection({ configPath }), /Exact installed package pin required: @verndale\/accessibility-standards@3\.1\.0/);
  } finally { await rm(root, { recursive: true, force: true }); }
});
