...
digest: "c7d1a449a8bfb334f4e422da9166dee42ef82dad276daa0f226a9576771fb1ee"
---
...
The initial `v1.0.0` publication [failed](https://github.com/verndale/accessibility-standards/actions/runs/33325307696): npm OIDC reported package-not-found and fallback authentication failed with `EOTP`. The Git tag exists while the npm package is absent. The workflow is converted to tokenless trusted publishing modeled on ui-design-library. The one-time `1.0.0` bootstrap and npm trusted-publisher configuration are external release steps; this schema/authority change is the next breaking `2.0.0` contract and must never emit `1.0.0` provenance. Release preflight requires the post-`v1.0.0` commit range to calculate exactly `2.0.0`, so the implementation PR must be squash-merged with a conventional breaking title such as `feat(standards)!: add deterministic UI pattern accessibility bindings`; a `prepack` contract guard catches any semantic-release rewrite that disagrees with the source contract.
...
- The `v1.0.0` tag exists but npm publication is absent; npm trusted-publisher configuration and one-time bootstrap remain external prerequisites before the breaking 2.0.0 release.
