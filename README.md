...
The current schema-3 projection targets `@verndale/accessibility-standards@3.1.0`. Schema 3 adds criterion-level standards references and a complete WCAG 2.2 Level A/AA coverage inventory to the existing deterministic UI-binding, evidence, and provenance outputs; version 3.1 expands that inventory with additive semantic-gap coverage and discriminator-gated APG patterns.
