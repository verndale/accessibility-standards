...
Generic UI names do not establish advanced APG behavior by themselves. The `toggle`, `modal`, and `button-group` bindings retain their existing field, dialog, or baseline semantic outcomes. They add `pattern.switch` only for `component.toggle_model = "switch"`, `pattern.alert-dialog` only for `component.dialog_purpose = "urgent-response"`, and `pattern.toolbar` only for `component.control_group_model = "toolbar"`. Keep these discriminator values distinct from the product decisions inside each selected pattern; the discriminator chooses the interaction contract, while product decisions complete that contract.
